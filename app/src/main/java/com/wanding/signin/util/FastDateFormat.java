...
public class FastDateFormat extends Format {
	...
	private static final Map C_INSTANCE_CACHE = new HashMap(7);
    @SuppressWarnings("rawtypes")
    private static final Map C_DATE_INSTANCE_CACHE = new HashMap(7);
    @SuppressWarnings("rawtypes")
    private static final Map C_TIME_INSTANCE_CACHE = new HashMap(7);
    @SuppressWarnings("rawtypes")
    private static final Map C_DATETIME_INSTANCE_CACHE = new HashMap(7);
    @SuppressWarnings("rawtypes")
    private static final Map C_TIME_ZONE_DISPLAY_CACHE = new HashMap(7);
	...
	public static synchronized FastDateFormat getInstance(String pattern, TimeZone timeZone, Locale locale) {
        ...
        FastDateFormat format = (FastDateFormat) C_INSTANCE_CACHE.get(emptyFormat);
        if (format == null) {
            format = emptyFormat;
            format.init();  // convert shell format into usable one
            C_INSTANCE_CACHE.put(format, format);  // this is OK!
        }
        return format;
    }
	...
	public static synchronized FastDateFormat getDateInstance(int style, TimeZone timeZone, Locale locale) {
        ...
        FastDateFormat format = (FastDateFormat) C_DATE_INSTANCE_CACHE.get(key);
        if (format == null) {
            try {
                SimpleDateFormat formatter = (SimpleDateFormat) DateFormat.getDateInstance(style, locale);
                String pattern = formatter.toPattern();
                format = getInstance(pattern, timeZone, locale);
                C_DATE_INSTANCE_CACHE.put(key, format);
	...
	public static synchronized FastDateFormat getTimeInstance(int style, TimeZone timeZone, Locale locale) {
        ...
        FastDateFormat format = (FastDateFormat) C_TIME_INSTANCE_CACHE.get(key);
        if (format == null) {
            if (locale == null) {
                locale = Locale.getDefault();
            }
            ...
            try {
                ...
                String pattern = formatter.toPattern();
                format = getInstance(pattern, timeZone, locale);
                C_TIME_INSTANCE_CACHE.put(key, format);
	...
	public static synchronized FastDateFormat getDateTimeInstance(int dateStyle, int timeStyle, TimeZone timeZone,
        ...
        FastDateFormat format = (FastDateFormat) C_DATETIME_INSTANCE_CACHE.get(key);
        if (format == null) {
            try {
                ...
                String pattern = formatter.toPattern();
                format = getInstance(pattern, timeZone, locale);
                C_DATETIME_INSTANCE_CACHE.put(key, format);
	...
	static synchronized String getTimeZoneDisplay(TimeZone tz, boolean daylight, int style, Locale locale) {
        ...
        if (value == null) {
            ...
            C_TIME_ZONE_DISPLAY_CACHE.put(key, value);
        }
        return value;
    }
